...
public class RestCustomerController {
	...
	public Object getToken(@PathVariable String email) {
		...
		if (customer == null) {
			...
			Email newEmail = new Email(email, "Token",
					"You've requested a password reset for your Book2ball account login. Please enter the token below in the Book2ball app, and you'll be able to create a new password. \r\n"
							+ "\r\n" + "Token: " + token + "\r\n\n\n" +
	...
	// new Booking(id, bookingDate, bookingType, status, startDateTime,
	// endDateTime));
	@RequestMapping(value = "/update/{username}/{password}/{firstName}/{lastName}/{email}/{contactNumber}/{startDate}/{endDate}/{status}/{originate}", method = {
			RequestMethod.OPTIONS, RequestMethod.POST })
	public Object updateCustomer(@PathVariable String username, @PathVariable String password,
			@PathVariable String firstName, @PathVariable String lastName, @PathVariable String email,
			@PathVariable String contactNumber, @PathVariable String startDate, @PathVariable String endDate,
			@PathVariable String status, @PathVariable String originate) {
		...
		boolean saved = customerDAO.updateCustomer(customer);
		System.out.println("Email saved is: " + customer.getEmail());
		if (saved) {
			return "success";
		} else {
